...
JWT significa Json Web Token. Ele é um código gerado e salva no navegador que, entre outras coisas, guarda as informações de usuário (exemplo: `{id: 1, email: "Sid Moreira"}`)
...
O token em si é dividido em três partes. As mais importantes para nós são as duas últimas: o Payload e o Signature.

O Payload é a parte do meio. É ali que fica armazenado informações como id  e email do usuário.

O Signature é a terceira parte. Ali fica armazenando o "segredo", que é um código que cada sistema tem o seu. Ele garante que aquele token pertence aquele sistema. Se for modifica, por exemplo, ele é invalidado. Isso evita que uma pessoa pague qualquer token e tente usar em nosso sistema.

como utilizar no nosso projeto? vamos lá?

1 - primeiro é preciso instalar as dependeências:
...
2.1 - importar tudo que é necessário: o jwt e também o dotenv(pois teremos váriáveis importantes lá)
```ts
    import jwt from 'jsonwebtoken'
    import * as dotenv from 'dotenv'
```

2.2 - Precisamos carregar as variáveis do .env para que o objeto precess.env (ele é que nos fornece os valores depois)

```ts
    dotenv.config() // sem isso aqu, não temos acesso as variáveis do .env

    // pegaos então as variáveis do .env
...
```ts
    ...
    JWT_SECRET=minhaChaveSecreta // serve para converter depois assinatura do token (signature)
...
```ts
    ...
    // Nosso método recebe por parâmetro um objeto que deve ter id e eamil (por causa da nossa interface)
    export function generateToken(payload:Payload){
        // O método sign() da biblioteca do jwt serve para criar um novo token.
        // para isso, possamos para ele, nestá ordem:
...
EXTRA: se quiser testar, no mesmo arquivo você pode chamar os dois métodos, primeiro generate e depois o verify:

```ts
    // gere um token com as infor que quiser
...
4.1 - Adicionar uma extensão da classe `Error` que vamos dar o nome de `UnauthorizedError` (Unauthorizer = não autorizado). Adicione a seguinte linha em `UserService.ts` :

```ts
    // Não é obrigatório criar essa classe filha de Error. Porém, ao fazermos isso, quando este erro for lançado, sabemos EXATANENTE do que se trata.
    export class UnauthorizedError extends Error {}
```

4.2 - Vamos adicionar o método de login dentro de `UserService.ts`. Este método vai receber um email e uma senha, validar se existe o email usando o `UserReposiry.findByEmail()` que criamos há pouco, depois valida atráves do método bcrypt.compare() se a senha bate com a criptografia dela no banco, e, se tudo estiver de acordo, ai sim gera um token chamando o método `jwt.generateToken()` que nós criamos antes. Então, no arquivo `UserSevice.ts` crie:
...
Passo 5 - agora, depois de Services, vamos para a camada Controller, onde vamos criar um arquivos chamado `AuthController.ts`. Ele ficará responsável pela parte do login. Dentro dele, insira: 
...
Passo 6 - Agora, precisamos criar a rota do Auth. Porém, ao contrário do que fazíamos antes, não vamos por todas as rotas em um único arquivos. vamos dicidi-las entre: rotas de auth, rotas de user, rotas de post e ai um arquivo principal que reúne todas elas, Vamos lá

6.1 - em `routes`, crie a um arquivos chamado `auth.routes.ts`. Dentro dele, coloque as importações:
...
```ts
    ...
    const router = Router()
    // O index agora chama as rotas criadas em auth.router.ts
